...
def main():
    ...
    for mu_A in muArange:   
        def singlepi(i):
            for mu_N in muNrange:
                for FWER in FWERrange:
                    pi = pirange[i]
                    filename_pre = 'MN%.2f_MA%.1f_tau%.2f_lbd%.2f_Si1.0_FWER%d_NH%d_ND%d_L%d_R%.3f_PM%.2f_alpha0%.2f_NR%d' % (mu_N, mu_A, args.tau, args.lbd, FWER, args.num_hyp, 1, args.markov_lag, non_range[i], pi, args.alpha0, args.num_runs)
                    ...
                    if all_filenames == []:
                        print("Running experiment for FWER procedure %s and pi %.1f and muN %.1f  and muA %.1f with lag %d and alpha0 %.2f and tau %.2f and lbd %.2f and non_range %.3f" % (proc_list[FWER-1], pi, mu_N, mu_A, args.markov_lag, args.alpha0, args.tau, args.lbd, non_range[i]))
                        run_single(non_range[i], args.num_runs, args.num_hyp, 1, mu_N, mu_A, args.tau, args.lbd, pi, args.alpha0, args.markov_lag, FWER)
                    else:
                        print("Experiments for FWER procedure %s and pi %.1f and muN %.1f and muA %.1f with lag %d and alpha0 %.2f and tau %.2f and lbd %.2f and non_range %.3f are already run" % (proc_list[FWER-1], pi, mu_N, mu_A, args.markov_lag, args.alpha0, args.tau, args.lbd, non_range[i]))
        with Pool() as p:
        ...
        for style in plotstyle:
            plot_results(style, 0, FWERrange, pirange, non_range, hyprange, args.tau, args.lbd, muNrange, mu_A, 1, args.num_hyp, args.num_runs, args.markov_lag, args.alpha0, mode)      
...
if __name__ == "__main__":
    ...
    parser.add_argument('--FWERrange', type=str, default = "1, 8, 12, 4") # choice of algorithms
