...
class adaptive_discarding_sidak_proc:

	def __init__(self, alpha0, numhpy, gamma_vec_exponent, tau, lbd, markov_lag=0):
	...
	def run_proc(self, pvec):
		...
		for k in range(numhpy):
			...
			if pvec[k]<=self.tau * self.lbd:
				C[k] = 1		
			beta = self.tau*(1-self.lbd)*self.gamma_vec[int(sum(S[:(k+1)])-sum(C[:k])-1)]
