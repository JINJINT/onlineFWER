...
def plot_best(pi, muN, muA, FWER, two_sided = False):
    ...
    taulist = np.round(np.arange(0.05, 1, 0.05),2)
    ...
    heatmapplot("$\lambda$", "$\\tau$", TDR_av, plot_dirname, filenameexp, lbdlist, taulist, 0, 1, "") 

    # # the heatmap of empirical power gap
    # heatmapplot("$\lambda$", "$\\tau$", TDR_gap_av, plot_dirname, filenameexp, lbd_list, tau_list, 0, 1, "power") 

    # the heatmap of empirical FWER
    # heatmapplot("$\lambda$", "$\\tau$", FWER_av, plot_dirname, filenameexp, lbdlist, taulist, 0, 0.2, "FWER") 

    # # the heatmap of empirical FWER bench
    # heatmapplot("$\lambda$", "$\\tau$", FWER_bench_av, plot_dirname, filenameexp, lbd_list, tau_list, 0, 0.2, "FWERbench") 




def main():
    for pi in np.arange(0.1,1,0.2):
        plot_best(pi, -1, 4, 4, two_sided = False)
        plot_best(pi, 0, 4, 4, two_sided = False)
